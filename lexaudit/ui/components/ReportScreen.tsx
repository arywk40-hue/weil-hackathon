...
function formatChainStatus(status?: string) {
  ...
  return status.replace(/^TransactionStatus\./, '').replace(/_/g, ' ')
}
...
export default function ReportScreen() {
  ...
  const chainReference = result.chain_reference ?? result.tx_hash ?? result.batch_id
  const chainReferenceLabel = result.chain_reference_type === 'batch_id' ? 'Batch ID' : 'Tx Hash'
  const chainStatus = formatChainStatus(result.chain_status)
  const chainConfirmed =
    Boolean(result.block_height) ||
    result.chain_status === 'TransactionStatus.CONFIRMED' ||
    result.chain_status === 'TransactionStatus.FINALIZED'
  const chainPending = result.chain_status === 'TransactionStatus.IN_PROGRESS'
  const chainTone = chainConfirmed
    ? 'text-volt/80'
    : chainPending
      ? 'text-amber-300/80'
      : chainReference
  ...
  const downloadReport = () => {
    const explorerLine = result.tx_explorer_url
      ? `Explorer: ${result.tx_explorer_url}`
      : ''
    const chainReferenceLine = chainReference
      ? `${chainReferenceLabel}: ${chainReference}`
      : `Weilchain Status: ${chainStatus}`
    const blockHeightLine = result.block_height ? `Block Height: ${result.block_height}` : ''
    ...
    <div className="min-h-screen pt-24 pb-16 px-6 max-w-4xl mx-auto">
      ...
      <div className="glass rounded-2xl p-5 mb-6 grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-white/30 text-xs font-mono">{chainReference ? chainReferenceLabel : 'Weilchain Status'}</span>
          </div>
          {chainReference && result.tx_explorer_url ? (
            <a
              href={result.tx_explorer_url}
              target="_blank"
              rel="noopener noreferrer"
              className="font-mono text-sm text-volt/80 hover:text-volt transition-colors truncate block"
              title={chainReference}
            >
              {chainReference.slice(0, 10)}…
            </a>
          ) : chainReference ? (
            <div className={`font-mono text-sm truncate ${chainTone}`} title={chainReference}>
              {chainReference}
            </div>
          ) : (
            <div className={`font-mono text-sm ${chainTone}`}>{chainStatus}</div>
          )}
          {result.block_height ? (
            <div className="font-mono text-[10px] text-white/30 mt-1">Block {result.block_height}</div>
          ) : null}
        </div>
      </div>
          ...
          <div className="border-t border-white/5 p-5">
            ...
            <div className="flex items-center gap-4 mb-4 glass rounded-xl p-3">
              ...
              <div className="flex-1">
                <div className="font-mono text-xs text-white/30">Weilchain Status</div>
                <div className={`font-mono text-xs mt-0.5 ${chainTone}`}>
                  {chainStatus}
                </div>
                {chainReference ? (
                  <div className="font-mono text-[11px] text-white/40 mt-1 break-all">
                    {chainReferenceLabel}: {chainReference}
                  </div>
                ) : null}
                {result.block_height ? (
      ...
      <div className="glass rounded-xl p-4 border border-volt/10 flex items-start gap-3">
        ...
        <p className="text-white/30 text-xs font-mono leading-relaxed">
          {chainConfirmed
            ? <>All {result.audit_events.length} audit events plus the final <span className="text-volt/60">AUDIT_COMPLETE</span> record are cryptographically signed on Weilchain. The final chain status is <span className="text-volt/60">{chainStatus}</span>{chainReference ? <> with {chainReferenceLabel.toLowerCase()} <span className="text-volt/60">{chainReference}</span></> : null}.</>
            : chainPending
              ? <>The final audit write has been submitted to Weilchain and is awaiting confirmation metadata from the node.</>
              : <>Audit trail is captured locally, but this SDK version did not return a final tx hash. When available, LexAudit now surfaces the Weilchain status and batch reference instead.</>
          }
        </p>
      </div>
    </div>
