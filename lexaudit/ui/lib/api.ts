...
const API_BASE = (process.env.NEXT_PUBLIC_API_URL || process.env.NEXT_PUBLIC_LEXAUDIT_API_BASE || 'http://localhost:8000').replace(/\/$/, '')
...
type BackendPayload = {
  ...
  tx_idx?: number | string | null
  chain_reference?: string | null
  chain_reference_type?: string | null
  state?: {
    filename?: string
...
function getReplayDelay(eventCount: number) {
  if (eventCount <= 0) return 0
  const budgetMs = 900
  const maxDelayMs = 80
  const minDelayMs = 16
  return Math.min(maxDelayMs, Math.max(minDelayMs, Math.floor(budgetMs / eventCount)))
}
...
function toAnalysisResult(payload: BackendPayload, fallbackFilename: string): AnalysisResult {
  ...
  const chainStatus =
    (typeof payload.chain_status === 'string' && payload.chain_status) ||
    (typeof payload.report_json?.chain_status === 'string' && payload.report_json.chain_status) ||
    undefined

  const batchId =
    (typeof payload.batch_id === 'string' && payload.batch_id) ||
    (typeof payload.report_json?.batch_id === 'string' && payload.report_json.batch_id) ||
    undefined

  const rawBlockHeight = payload.block_height ?? payload.report_json?.block_height
  const blockHeight =
    typeof rawBlockHeight === 'number'
      ? rawBlockHeight
      : (typeof rawBlockHeight === 'string' && rawBlockHeight.trim()
        ? Number(rawBlockHeight)
        : undefined)

  const rawTxIdx = payload.tx_idx ?? payload.report_json?.tx_idx
  const txIdx =
    typeof rawTxIdx === 'number'
      ? rawTxIdx
      : (typeof rawTxIdx === 'string' && rawTxIdx.trim()
        ? Number(rawTxIdx)
        : undefined)

  const chainReference =
    (typeof payload.chain_reference === 'string' && payload.chain_reference) ||
    (typeof payload.report_json?.chain_reference === 'string' && payload.report_json.chain_reference) ||
    txHash ||
    batchId ||
    undefined

  const chainReferenceType =
    (typeof payload.chain_reference_type === 'string' && payload.chain_reference_type) ||
    (typeof payload.report_json?.chain_reference_type === 'string' && payload.report_json.chain_reference_type) ||
    (txHash ? 'tx_hash' : batchId ? 'batch_id' : undefined)
  ...
  return {
    ...
    tx_idx: Number.isFinite(txIdx) ? txIdx : undefined,
    chain_reference: chainReference,
    chain_reference_type: chainReferenceType,
  }
}
...
export async function streamAnalysis(
  ...
  const replayDelay = getReplayDelay(result.audit_events.length)
